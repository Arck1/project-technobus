                        ...
                        if (hour !== "" && timeSort[hour][index].min !== "") {
                            ...
                            if (weekCount < 5) {
                                ...
                                if (weekSelect[itIsSuperLongIteratorButInVeryUseful].length === 0) {
                                    ...
                                    for (var _i3 = 0; _i3 < weekSelect.length; _i3++) {
                                        if (weekSelect[_i3].length > 0) {
                                            specialInfo += inWeekDays[(_i3 + 1) % 7] + " ";
                                        }
                                    }
                                }
                            }
...
$(function () {
    ...
    function setDate() {
        ...
        if (index < listTo.length - 1) {
            ...
            while (nextIndex < listTo.length && $(listTo[nextIndex]).hasClass('shadow')) {
                ...
                var _nextTime$innerText$s = nextTime.innerText.split(':'),
                    ...
                    _h3 = _nextTime$innerText$s2[0],
                    _m3 = _nextTime$innerText$s2[1];

                var nextTimeInMinutes = parseInt(_h3) * 60 + parseInt(_m3);
        ...
        if (minDiffFromTime >= 10 && minDiffFromTime < 200) {
            $(selected_item).before("<li class=\"break fone\"><div class=\"time-info\">" + "<div class=\"info\">Перерыв " + minDiffFromTime + " минут</div>" + "<div class=\"desc\">Воспользуйтесь общественным транспортом</div></div></li>");
        ...
        for (var _i = 0; _i < listFrom.length; _i++) {
            ...
            var _divTime3$innerText$s = _divTime3.innerText.split(':'),
                ...
                _h4 = _divTime3$innerText$s2[0],
                _m4 = _divTime3$innerText$s2[1];

            var _hm2 = parseInt(_h4) * 60 + parseInt(_m4);

            if (_hm2 > timeInMins && minDiffFromTime > Math.abs(_hm2 - timeInMins) && !$(_li).hasClass('shadow')) {
                selected_item = _li;
                index = _i;
        ...
        if (minDiffFromTime === 90000) {
            ...
            var _divTime4$innerText$s = _divTime4.innerText.split(':'),
                ...
                _h5 = _divTime4$innerText$s2[0],
                _m5 = _divTime4$innerText$s2[1];

            var _hm3 = parseInt(_h5) * 60 + parseInt(_m5);
        ...
        if (index < listFrom.length - 1) {
            ...
            while (_nextIndex < listFrom.length && $(listFrom[_nextIndex]).hasClass('shadow')) {
                ...
                var _nextTime$innerText$s3 = _nextTime.innerText.split(':'),
                    ...
                    _h6 = _nextTime$innerText$s4[0],
                    _m6 = _nextTime$innerText$s4[1];

                var _nextTimeInMinutes = parseInt(_h6) * 60 + parseInt(_m6);
        ...
        if (minDiffFromTime >= 10 && minDiffFromTime < 200) {
            $(selected_item).before("<li class=\"break ftwo\"><div class=\"time-info\">" + "<div class=\"info\">Перерыв " + minDiffFromTime + " минут</div>" + "<div class=\"desc\">Воспользуйтесь общественным транспортом</div></div></li>");
...
$(function switchs() {
    ...
    $("#toTechn").click(function () {
        ...
        if (currentWindow == 1) {
            ...
            $('#timelineDirection').animate({ scrollTop: $("#scheduleList0 li.next").offset().top - $("#scheduleList0").offset().top - 60 });
        } else if (currentWindow == 2) transition2();else if (currentWindow == 3) transition3();
